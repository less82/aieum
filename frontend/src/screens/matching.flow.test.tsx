...
describe("매칭 플로우 (단일 공개 → 지도/코스 → 코스 선택 → 성공/실패)", () => {
  it("B코스 선택(상대와 동일)하면 매칭 성공 → 데이트 준비", async () => {
    ...
    // 코스 A/B 미리보기 로드 → B코스 선택
    expect((await screen.findAllByText("🎨 감성 카페 코스")).length).toBeGreaterThan(0);
    fireEvent.click(screen.getByText("🧗 액티브 코스"));
    const cta = await screen.findByText(/B코스로 데이트 신청/);
    ...
    expect(st.session?.courseId).toBe("cb");
    expect(st.session?.state).toBe("scheduled");
    expect(st.credits).toBe(2);
  });

  it("A코스 선택(상대와 불일치)하면 매칭 실패 → 다시 신청 가능", async () => {
    useDemo.getState().setOnboarding(
      { EI: 0.5, JP: 0.5, active: 0.5, values: 0.5 },
      { keywords: ["#계획형"], summary: "요약" }
    );
    ...
    // 기본 A코스로 신청 (상대는 B를 선택 → 불일치)
    fireEvent.click(await screen.findByText(/A코스로 데이트 신청/));
